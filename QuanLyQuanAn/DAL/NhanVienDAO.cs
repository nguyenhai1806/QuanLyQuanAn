...
namespace QuanLyQuanAn.DAL
{
    class NhanVienDAO
    {
        ...
        public bool ThemNhanVien(int maNhomNV, string tenDanhNhap, string matKhau, string hoTen, string gioiTinh,
            DateTime ngaySinh, string diaChi, string soDienThoai, bool trangThai)
        {
            string sql =
                "EXEC dbo.P_ThemNhanVien @MaNhomNV , @TenDanhNhap , @MatKhau , @HoTen , @GioiTinh , @NgaySinh , @DiaChi , @SDT , @TrangThai ";
            int result = DataProvider.Instance.ExcuteNonQuery(sql,
                new Object[]
                    {maNhomNV, tenDanhNhap, matKhau, hoTen, gioiTinh, ngaySinh, diaChi, soDienThoai, trangThai});
            return result > 0;
        }

        public bool SuaNhanVien(int maNV, int maNhomNV, string hoTen, string gioiTinh, DateTime ngaySinh, string diaChi,
            string soDienThoai, bool trangThai)
        {
            bool result = false;
            string query =
                "EXEC dbo.P_SuaNhanVien @MaNV , @MaNhomNV , @HoTen , @GioiTinh , @NgaySinh , @DiaChi , @SDT , @TrangThai ";
            result = DataProvider.Instance.ExcuteNonQuery(query,
                new object[] {maNV, maNhomNV, hoTen, gioiTinh, ngaySinh, diaChi, soDienThoai, trangThai}) > 0;
            return result;
        }
        ...
        public bool UpdateInfo(int maNV, string gioiTinh, DateTime ngaySinh, string DiaChi, string sdt)
        {
            ...
            result = DataProvider.Instance.ExcuteNonQuery(query, new object[] {maNV, gioiTinh, ngaySinh, DiaChi, sdt}) >
                     0;
            return result;
        }
        public List<NhanVien> TimNhanVien(string value)
        {
            ...
            string query = String.Format("SELECT * FROM NhanVien where SDT like N'%{0}%' or HoTen Like N'%{1}%' Or TenDangNhap  like N'%{2}%' ", value, value,value);
            DataTable data = DataProvider.Instance.ExcuteQuery(query);
